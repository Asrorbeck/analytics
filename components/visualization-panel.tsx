      ...
      if (selectedXColumn && numericColumns.includes(selectedXColumn)) {
        return data
          .map((row) => ({
            ...
            color: selectedColorColumn ? String(row[selectedColorColumn] || "Unknown") : undefined,
          }))
    ...
    if (chartType === "bar") {
      return (
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} layout={orientation === "horizontal" ? "vertical" : "default"}>
        ...
        <div className="w-full h-full overflow-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {uniqueY.map((y) => (
                <tr key={String(y)}>
                  <td className="px-2 py-1 text-xs font-semibold">{String(y).slice(0, 10)}</td>
                  {uniqueX.map((x) => {
                    const cell = chartData.find((d: any) => d.x === x && d.y === y)
                    const value = cell?.value || 0
                    const intensity = Math.abs(value)
                    const color = value >= 0 ? `rgba(34, 197, 94, ${intensity})` : `rgba(239, 68, 68, ${intensity})`
                    return (
                      <td
                        key={`${x}-${y}`}
      ...
      return (
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={chartData}>
            ...
            <Line yAxisId="right" type="monotone" dataKey="cumulative" stroke="#82ca9d" strokeWidth={2} />
          </ComposedChart>
        </ResponsiveContainer>
      )
      ...
      return (
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={chartData}>
            ...
            <Tooltip />
            <Legend />
            <Area type="monotone" dataKey={selectedYColumn} stroke="#8884d8" fill="#8884d8" fillOpacity={0.3} />
          </AreaChart>
        </ResponsiveContainer>
      )
    } else if (chartType === "histogram") {
    ...
    <div className="space-y-6">
      ...
      <div className="card-subtle p-6 rounded-lg border border-border">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {(chartType === "bar" || chartType === "line" || chartType === "pie" || chartType === "area" || chartType === "pareto" || chartType === "box") && (
            <div>
          ...
          {chartType === "scatter" && (
            <div>
              <label className="text-sm text-foreground/70 block mb-2">X Axis</label>
              <select
                value={selectedXColumn}
                onChange={(e) => setSelectedXColumn(e.target.value)}
                className="w-full px-3 py-2 bg-input border border-border rounded-lg text-foreground focus:outline-none focus:border-primary text-sm"
          ...
          {chartType === "scatter" && (
            <div>
              <label className="text-sm text-foreground/70 block mb-2">Color By (Optional)</label>
              <select
          ...
          {chartType === "bar" && (
            <div>
              <label className="text-sm text-foreground/70 block mb-2">{t.orientation}</label>
              <select
                value={orientation}
          ...
          {chartType === "histogram" && (
            <div>
              <label className="text-sm text-foreground/70 block mb-2">{t.bins}</label>
              <input
                type="range"
                min="10"
                max="100"
